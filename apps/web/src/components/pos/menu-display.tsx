...
import { Plus, Minus, Pizza, Coffee, CupSoda, Croissant, Utensils, IceCream, ShoppingBag } from "lucide-react";
...
export function MenuDisplay({ catalog }: { catalog: any[] }) {
  const [selectedCategoryId, setSelectedCategoryId] = useState<string | null>(catalog[0]?.id || null);
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      <h1 className="text-3xl font-bold tracking-tight text-foreground mb-6">{t("pos.categories")}</h1>
        ...
        <div className="flex w-max space-x-3 px-1">
          {catalog.map((category) => {
            ...
            return (
              ...
              >
                <div className={`p-2.5 rounded-full ${isActive ? 'bg-primary/10' : 'bg-muted/50'}`}>
      ...
      <div className="flex items-center justify-between mb-4 mt-2">
         <h2 className="text-xl font-semibold tracking-tight text-foreground">{activeCategory?.name || t("pos.menu")}</h2>
         <span className="text-sm font-medium text-muted-foreground">{t("pos.showingItems", { count: products.length })}</span>
      </div>
          ...
          >
            ...
            <div className="flex items-start gap-4 mb-3">
              <div className="w-[85px] h-[85px] shrink-0 rounded-2xl overflow-hidden relative bg-muted/30">
                <img
                  src={product.imageUrl || 'https://images.unsplash.com/photo-1572490122747-3968b75cc699?w=400&h=400&fit=crop'}
                  alt={product.name}
